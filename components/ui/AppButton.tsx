...
export function AppButton({ label, loading, variant = "primary", icon, disabled, onPressIn, onPressOut, ...props }: Props) {
  ...
  return <AnimatedPressable {...props} disabled={disabled || loading} onPressIn={(event) => { scale.value = withTiming(0.97, { duration: 90 }); onPressIn?.(event); }} onPressOut={(event) => { scale.value = withTiming(1, { duration: 120 }); onPressOut?.(event); }} style={[styles.base, palette, (disabled || loading) && styles.disabled, animated]} accessibilityRole="button" accessibilityState={{ disabled: !!disabled, busy: !!loading }}>
    {loading ? <ActivityIndicator color={variant === "secondary" || variant === "ghost" ? "#FF8700" : "#FFFFFF"} /> : <View style={styles.content}>{icon}<Text style={[styles.label, (variant === "secondary" || variant === "ghost") && styles.darkLabel]}>{label}</Text></View>}
  </AnimatedPressable>;
}
const styles = StyleSheet.create({ base: { minHeight: 50, borderRadius: radius.md, alignItems: "center", justifyContent: "center", paddingHorizontal: spacing.lg }, content: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: spacing.sm }, primary: { backgroundColor: "#FF8700" }, secondary: { backgroundColor: "#FFF7ED", borderWidth: 1, borderColor: "#FED7AA" }, danger: { backgroundColor: "#DC2626" }, ghost: { backgroundColor: "transparent" }, disabled: { opacity: 0.55 }, label: { color: "#FFFFFF", ...typography.body, fontWeight: "800" }, darkLabel: { color: "#E86F00" } });
