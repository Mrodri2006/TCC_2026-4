...
export function StateView({ kind, title, message, actionLabel, onAction, icon }: Props) {
  ...
  const visual = kind === "loading" ? <ActivityIndicator size="large" color="#2563EB" /> : (icon ?? (kind === "empty" ? <Inbox size={34} color="#2563EB" /> : <AlertTriangle size={34} color={kind === "offline" ? "#F59E0B" : "#DC2626"} />));
