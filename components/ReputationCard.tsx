...
export function ReputationCard({ data }: { data: any }) {
  ...
  return <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
    ...
    <View style={styles.metrics}>
      ...
      <View style={styles.metric}><Clock3 size={17} color="#2563EB" /><Text style={[styles.value, { color: theme.textPrimary }]}>{reputation.responseMinutes === null ? "—" : `${reputation.responseMinutes} min`}</Text><Text style={[styles.label, { color: theme.textMuted }]}>resposta</Text></View>
    </View>
...
const styles = StyleSheet.create({ card: { marginHorizontal: 16, marginBottom: 16, padding: 16, borderRadius: 18, borderWidth: 1 }, header: { flexDirection: "row", alignItems: "center", gap: 8 }, title: { fontSize: 16, fontWeight: "900" }, metrics: { flexDirection: "row", marginTop: 16 }, metric: { flex: 1, alignItems: "center" }, value: { fontSize: 16, fontWeight: "900", marginTop: 4 }, label: { fontSize: 10, marginTop: 2 }, rates: { textAlign: "center", fontSize: 11, fontWeight: "700", marginTop: 14 }, badges: { flexDirection: "row", flexWrap: "wrap", justifyContent: "center", gap: 6, marginTop: 12 }, badge: { color: "#1D4ED8", backgroundColor: "#DBEAFE", fontSize: 10, fontWeight: "800", paddingHorizontal: 9, paddingVertical: 5, borderRadius: 999 } });
